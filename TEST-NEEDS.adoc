...
80 tests passing (`+cargo test+`, 0 failures) + 14 benchmarks compiled.
All CRG C categories met for `+dipstick/services/collector+`.
...
** tests/unit_tests.rs: model creation, serialization, checksum, word
count
...
** tests/reflexive_tests.rs: round-trip serialization, model
reconstruction
...
** benches/collector_bench.rs: Criterion benchmarks for models and
checksums
...
* *MEDIUM* — Additional services need test coverage, but core collector
service at CRG C.
