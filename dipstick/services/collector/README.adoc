...
Scraping policy/terms/community guidelines pages - Fetching via platform
APIs where available - Detecting changes through content checksumming -
...
* *Scheduled collection*: Cron-based automatic collection (default:
every 15 minutes)
...
[arabic]
. Scheduler triggers collection cycle (or manual HTTP trigger)
. Fetch active platforms from database
. For each platform (up to MAX_CONCURRENT_COLLECTIONS parallel):
...
Changes detected by comparing SHA256 checksums of content: - *Identical
checksum*: No change, record snapshot only - *Different checksum*:
Change detected, create policy_change record
